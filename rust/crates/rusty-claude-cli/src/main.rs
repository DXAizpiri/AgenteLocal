...
impl AnthropicRuntimeClient {
    fn new(
        ...
        let mut client = api::ProviderClient::from_model_with_anthropic_auth(&model, auth)?;
        if let api::ProviderClient::Anthropic(c) = client {
            client = api::ProviderClient::Anthropic(c.with_base_url(api::read_base_url()));
        }
